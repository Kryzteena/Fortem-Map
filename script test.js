document.addEventListener('DOMContentLoaded', () => {
  ...
  const warehouseColors = {
    'L07': 'blue', 'L08': 'red', 'L10': 'green',
    'V01': 'orange', 'V02': 'pink', 'V03': 'purple',
    'M03': 'yellow', 'M01': 'black'
  };
    ...
    Papa.parse(file, {
      ...
      complete: function (results) {
        ...
        data.forEach(row => {
          ...
          let iconType = cleanType === 'warehouse' ? 'fa-warehouse' : 'fa-truck';
  ...
  legend.onAdd = function () {
    ...
    for (const [id, color] of Object.entries(warehouseColors)) {
      div.innerHTML += `<i style="background:${color}; width:12px; height:12px; display:inline-block; margin-right:6px; border-radius:50%;"></i>${id}<br>`;
    }
    return div;
  };
  legend.addTo(map);
